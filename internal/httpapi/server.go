...
func New(app *appcore.App) *gin.Engine {
	...
	auth.POST("/resources/:id/check", s.checkResource)
	auth.POST("/resources/:id/reality-scan", s.scanReality)
	auth.POST("/resources/check-all", s.checkAllResources)
...
func (s *Server) login(c *gin.Context) {
	...
	if err != nil || !security.VerifyPassword(account.PasswordHash, input.Password) {
	...
	key := "session:" + token
	if err := s.app.Redis.HSet(c, key, "user_id", account.ID, "csrf", csrf).Err(); err != nil {
...
func (s *Server) logout(c *gin.Context) {
	if token, err := c.Cookie("panel_session"); err == nil {
		s.app.Redis.Del(c, "session:"+token)
	}
...
func (s *Server) me(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{"user": publicUser(c.MustGet("user").(*ent.User)), "csrfToken": c.GetString("csrf")})
}

func (s *Server) requireAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		token, err := c.Cookie("panel_session")
		if err != nil || token == "" {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
			return
		}
		values, err := s.app.Redis.HGetAll(c, "session:"+token).Result()
		if err != nil || len(values) == 0 {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
			return
		}
		uid, err := strconv.Atoi(values["user_id"])
		if err != nil {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
			return
		}
		account, err := s.app.DB.User.Get(c, uid)
		if err != nil || !account.Enabled {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
			return
		}
		if c.Request.Method != http.MethodGet && c.Request.Method != http.MethodHead && c.GetHeader("X-CSRF-Token") != values["csrf"] {
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{"error": "csrf_failed"})
			return
		}
		c.Set("user", account)
		c.Set("csrf", values["csrf"])
		c.Next()
	}
}

func publicUser(u *ent.User) gin.H {
	return gin.H{"id": u.ID, "username": u.Username, "role": u.Role, "mustChangePassword": u.MustChangePassword}
}
...
func (s *Server) listResources(c *gin.Context) {
	items, err := s.app.DB.Resource.Query().Order(ent.Asc(resource.FieldCode)).All(c)
	if err != nil {
	...
	for _, item := range items {
		result = append(result, resourceJSON(item))
	}
...
func (s *Server) createResource(c *gin.Context) {
	...
	if strings.TrimSpace(input.Code) == "" || strings.TrimSpace(input.Name) == "" || strings.TrimSpace(input.Host) == "" {
		c.JSON(http.StatusUnprocessableEntity, gin.H{"error": "required_fields"})
		return
	}
	token, err := s.app.Cipher.Encrypt(input.AccessToken)
	if err != nil {
	...
	if err != nil {
		c.JSON(http.StatusConflict, gin.H{"error": "resource_exists"})
		return
	}
	s.audit(c, "resource.create", "resource", strconv.Itoa(item.ID))
	c.JSON(http.StatusCreated, resourceJSON(item))
}
...
func (s *Server) updateResource(c *gin.Context) {
	...
	if input.AccessToken != "" {
		...
		update.SetAccessTokenCiphertext(token)
	}
	item, err := update.Save(c)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "resource_not_found"})
		return
	}
	s.audit(c, "resource.update", "resource", strconv.Itoa(item.ID))
	c.JSON(http.StatusOK, resourceJSON(item))
}
...
func (s *Server) deleteResource(c *gin.Context) {
	...
	if err := s.app.DB.Resource.DeleteOneID(id).Exec(c); err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "resource_not_found"})
		return
	}
	s.audit(c, "resource.delete", "resource", strconv.Itoa(id))
	c.JSON(http.StatusOK, gin.H{"ok": true})
}

func (s *Server) checkResource(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_id"})
		return
	}
	queued, err := s.enqueueResourceTask(c, tasks.TypeResourceCheck, id, "")
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "queue_failed", "message": err.Error()})
		return
	}
	c.JSON(http.StatusAccepted, queued)
}

func (s *Server) scanReality(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid_id"})
		return
	...
	queued, err := s.enqueueResourceTask(c, tasks.TypeRealityScan, id, input.Targets)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "queue_failed", "message": err.Error()})
		return
	}
...
func (s *Server) checkAllResources(c *gin.Context) {
	items, err := s.app.DB.Resource.Query().All(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "query_failed"})
		return
	}
	...
	for _, item := range items {
		queued, err := s.enqueueResourceTask(c, tasks.TypeResourceCheck, item.ID, "")
		if err == nil {
			jobs = append(jobs, queued)
		}
	}
	c.JSON(http.StatusAccepted, gin.H{"jobs": jobs, "count": len(jobs)})
}

func (s *Server) enqueueResourceTask(c *gin.Context, taskType string, resourceID int, targets string) (gin.H, error) {
	item, err := s.app.DB.Resource.Get(c, resourceID)
	if err != nil {
		return nil, fmt.Errorf("resource not found")
	}
	if item.ManagementURL == "" || item.AccessTokenCiphertext == "" {
		return nil, fmt.Errorf("management URL and access token are required")
	}
	key, err := security.RandomToken(12)
	if err != nil {
		return nil, err
	}
	label := "资源检测"
	if taskType == tasks.TypeRealityScan {
		label = "Reality 目标扫描"
	}
	jobRow, err := s.app.DB.Job.Create().SetJobKey("job_" + key).SetType(taskType).SetStatus("queued").SetMessage(label + "等待执行").SetPayload(map[string]any{"resourceId": resourceID, "resourceName": item.Name}).Save(c)
	if err != nil {
		return nil, err
	}
	task, err := tasks.New(taskType, tasks.Payload{JobID: jobRow.ID, ResourceID: resourceID, Targets: targets})
	if err != nil {
		return nil, err
	}
	if _, err = s.app.Queue.Enqueue(task); err != nil {
		_ = s.app.DB.Job.UpdateOneID(jobRow.ID).SetStatus("failed").SetMessage(err.Error()).Exec(c)
		return nil, err
	}
	s.audit(c, "job.enqueue", "job", strconv.Itoa(jobRow.ID))
	return gin.H{"id": jobRow.ID, "jobKey": jobRow.JobKey, "status": "queued"}, nil
}

func resourceJSON(item *ent.Resource) gin.H {
	return gin.H{"id": item.ID, "code": item.Code, "name": item.Name, "host": item.Host, "managementUrl": item.ManagementURL, "hasAccessToken": item.AccessTokenCiphertext != "", "status": item.Status, "nodeCount": item.NodeCount, "clientCount": item.ClientCount, "socksCount": item.SocksCount, "latencyMs": item.LatencyMs, "lastCheckedAt": item.LastCheckedAt}
}

func (s *Server) listJobs(c *gin.Context) {
	items, err := s.app.DB.Job.Query().Order(ent.Desc(job.FieldCreatedAt)).Limit(100).All(c)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "query_failed"})
		return
	}
...
func (s *Server) audit(c *gin.Context, action, targetType, targetID string) {
	account, _ := c.Get("user")
	uid := 0
	if account != nil {
		uid = account.(*ent.User).ID
	}
