...
func main() {
	...
	if err := application.DB.Schema.Create(ctx); err != nil { log.Fatal(err) }
	if len(os.Args) > 1 && os.Args[1] == "bootstrap-admin" { bootstrapAdmin(ctx, application); return }
	if err := application.Ready(ctx); err != nil { log.Fatal(err) }
	...
	select { case err := <-serverErr: log.Fatal(err); case <-signals: log.Println("shutdown requested") }
}
...
func bootstrapAdmin(ctx context.Context, application *appcore.App) {
	...
	if username == "" || password == "" { log.Fatal("ADMIN_USERNAME and ADMIN_PASSWORD are required") }
	exists, err := application.DB.User.Query().Where(user.UsernameEQ(username)).Exist(ctx); if err != nil { log.Fatal(err) }
	if exists { log.Fatal("administrator already exists") }
	hash, err := security.HashPassword(password); if err != nil { log.Fatal(err) }
	account, err := application.DB.User.Create().SetUsername(username).SetPasswordHash(hash).SetRole("owner").SetMustChangePassword(false).Save(ctx); if err != nil { log.Fatal(err) }
